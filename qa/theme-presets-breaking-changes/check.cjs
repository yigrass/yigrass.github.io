...
function mount(href = "https://example.test/", baseURI = "https://example.test/", options = {}) {
  let document;
  class Element {
    constructor(tag) {
  ...
  for (const file of ["assets/site-config.js", "assets/themes.js", "assets/project-routes.js", "assets/app.js"]) vm.runInContext(read(file), context, { filename: file });
  ...
  return { window, document, location, history, get, click, menu, drive, entry, win, tab, saved: () => JSON.parse(stored) };
}
...
for (const theme of themed.window.DESKTOP_THEMES) {
...
for (const theme of themed.window.DESKTOP_THEMES) {
  const fresh = mount("https://example.test/system-settings/", "https://example.test/", { saved: { themeId: theme.id, menuTransparency: 25, customCursor: true, floatingStars: false } });
...
for (const options of [{ saved: { themeId: "not-a-theme" } }, { rawStorage: "{invalid" }, { storageUnavailable: true }]) {
  const fresh = mount("https://example.test/system-settings/", "https://example.test/", options);
...
for (const theme of themed.window.DESKTOP_THEMES.filter(theme => theme.scheme === "dark")) {
...
check(!/legacyRoutes/.test(read("assets/site-config.js") + read("assets/project-routes.js")), "No legacy route compatibility remains in runtime configuration or router");
...
const result = { passed: checks.length, checks, limitation: "Actual application code with simulated DOM/history/storage and static CSS checks; not a browser layout, pointer or visual test." };
fs.writeFileSync(path.join(__dirname, "results.json"), JSON.stringify(result, null, 2) + "\n");
console.log("Passed " + checks.length + " theme, routing and regression checks.");
if (process.argv.includes("--serve")) require("./preview-server.cjs");
const previewArg = process.argv.indexOf("--preview-url");
if (previewArg !== -1) {
  const previewUrl = new URL(process.argv[previewArg + 1]);
  assert.equal(previewUrl.hostname, "127.0.0.1", "HTTP validation is loopback only");
  (async () => {
    const responses = [];
    for (const route of ["/", ...[...catalog.byPath.keys()].map(route => "/" + route), "/assets/themes.js", "/assets/app.js", "/assets/styles.css"]) {
      const response = await fetch(new URL(route, previewUrl), { signal: AbortSignal.timeout(5000) });
      const bytes = await response.arrayBuffer();
      assert.equal(response.status, 200, route); assert.ok(bytes.byteLength, route);
      responses.push({ route, status: response.status, bytes: bytes.byteLength });
    }
    for (const route of ["/novels/story-a/", "/games/game-b/"]) {
      const response = await fetch(new URL(route, previewUrl), { signal: AbortSignal.timeout(5000) });
      assert.equal(response.status, 404, "Removed route must not resolve: " + route);
      responses.push({ route, status: response.status });
    }
    fs.writeFileSync(path.join(__dirname, "http-results.json"), JSON.stringify({ url: previewUrl.href, responses, limitation: "Loopback HTTP only; not a browser or remote hosting test." }, null, 2) + "\n");
    console.log("Passed " + responses.length + " runtime and removed-route HTTP checks.");
