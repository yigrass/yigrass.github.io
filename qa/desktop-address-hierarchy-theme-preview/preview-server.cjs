...
server.listen(0, "127.0.0.1", async () => {
  ...
  const results = [];
  try {
    for (const route of ["/", ...[...byPath.keys()].map(route => "/" + route), "/assets/app.js", "/assets/project-routes.js", "/assets/styles.css", "/assets/pixel-ui/v1.3.0/scrollbar-arrow-up.png"]) {
    ...
    for (const route of ["/art/", "/qa/task-transitions.jsonl", "/project-task-tree.json"]) {
      const response = await fetch(url + route, { signal: AbortSignal.timeout(5000) });
