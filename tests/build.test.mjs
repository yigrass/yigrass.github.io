...
test('clean input copy builds without art or other projects; invalid release preserves previous dist', async t => {
  ...
  for (const directory of ['src', 'catalog', 'releases', 'assets', 'scripts', 'contracts']) await fs.cp(path.join(projectRoot, directory), path.join(fixture, directory), { recursive: true });
...
test('built release receipts match every published package file', async () => {
  ...
  for (const receipt of receipts) for (const file of receipt.files) {
    const bytes = await fs.readFile(path.join(projectRoot, 'dist/releases', receipt.id, file.path));
