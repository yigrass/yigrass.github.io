...
test('dist serves all deep entries, module imports and release bytes; maintenance paths are absent', async t => {
  ...
  const text = await fetch(new URL('releases/sh-tales/chapters/chapter-1.md', base)); assert.match(await text.text(), /第一章 · 借火/);
  ...
  assert.equal(index[0].iconPath, 'releases/sh-tales/images/sword.png');
...
test('clean input copy builds without art or other projects; invalid release preserves previous dist', async t => {
  ...
  await fs.unlink(path.join(fixture, 'releases/sh-tales/chapters/chapter-1.md'));
