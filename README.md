...
将真实项目加入 `works` 数组即可显示入口，每项填写唯一的 `id`、项目名称 `title`、分类 `category` 和项目地址 `url`。分类取值为 `novel`、`game` 或 `utility`，链接可以是站内相对路径或完整网页地址。小说采用文本文件图标，显示名称自动补上 `.txt`（已有该后缀时不重复添加）；点击会在新标签页打开完整项目，因此小说页面仍可包含插图。游戏使用光盘图标，工具使用工具面板图标。当前 `works` 仍为空，尚未提供真实项目。资源管理器分类及新图标检查见 `qa/explorer-project-categories.md`。音乐播放和完整 JRPG 主题不属于本版。
