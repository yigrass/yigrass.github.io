...
“资源管理器”提供左侧文件夹导航、地址选择栏、后退、前进和向上按钮。根目录先放置 A:（3.5 英寸软盘）、C:、D:（本地磁盘）及 E:（光盘驱动器），点击可进入空目录。文件夹树可以折叠，“文件夹”按钮可以隐藏左栏，便于窄屏查看。这些是网站内的虚拟目录，不读取访客电脑，也没有虚构容量或文件。关闭后重新打开会回到根目录。
...
用户已查看并认可初始桌面外壳。首次预览检查保留在 `qa/mvp-preview-check.md`，设置窗口与像素 UI 修改的检查见 `qa/system-settings-pixel-ui-review.md`，本次资源管理器、系统属性及新图标的检查见 `qa/explorer-system-properties-review.md`。历史报告中的环境与接入状态记录其当时情况；当前状态以任务树和后继报告为准。
