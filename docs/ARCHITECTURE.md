...
阅读器目录以清单顺序渲染，README 在所属层级最前面；节点折叠与文档选择分开处理。目录保留原有细分隔线，页签直接定位于正文面板左上角的留白处，不占用独立竖栏。梯形宽度引用 --scrollbar-size，底边采用 45° 斜线，单行双方向字符在短边中点对齐。页签不使用 classic-button；悬停、按住和键盘焦点仅显示随主题变色的虚线轮廓，SVG 遮罩覆盖斜边在内的整个梯形，不使用凹凸、位移或动画。标题栏下的菜单栏提供“文件”“查看”占位菜单，以及使用现有 back/forward 图标的常显翻章按钮；reader-menu.js 管理菜单定义、互斥展开、外部点击或焦点离开时关闭、Escape 返回按钮及菜单间方向键切换。全局翻章方向键只在该阅读器窗口处于前台、开始菜单和阅读器菜单关闭且未编辑输入控件时生效。ResizeObserver 根据菜单栏下面的实际阅读区高度更新末尾留白，使最后一行可以滚至顶部；关闭窗口时释放观察器及全部全局事件。用户书签、显示偏好和自定义快捷键尚未实现，与每书内部视觉配置分别规划；这些界面调整不改变小说成品合同。
