...
assets/                      网站自己的上线图片
contracts/novel-release-v1/   可独立复制的小说成品合同
...
网站目录决定虚拟磁盘、图标、窗口标题与地址，作品自己的 release.json 决定作品内容。小说 manifest 规则以 [合同](../contracts/novel-release-v1/README.md) 及其 Schema、校验器为准；本文件不另列一套字段规则。
...
window-manager 管理位置、尺寸、前台层级、最小化、最大化和关闭；应用渲染器只提供窗口内容。窗口内容可返回 dispose、setVisible 和 ready；关闭时取消正在读取的内容并清理监听，最小化保留实例和当前章节。章节读取使用请求序号避免旧响应覆盖新选择，正文以 textContent 渲染。
...
源码输出到 dist/app，素材到 dist/assets，作品目录和成品保留在 dist/catalog、dist/releases。首页及各应用、磁盘和作品深层入口由同一模板生成。art、contracts、docs、qa、tests、scripts、任务控制及 Git 元数据都不是发布输入。不存在仅将源文件移到 src、仍从仓库根直接发布的中间模式。
...
新小说章节不产生新的桌面窗口或章节级公开路径；当前作品 URL 保持不变，章节选择仅是阅读器内部状态。章节书签、阅读进度保存、富文本格式和自定义主题等功能在有明确需求时单独迭代。
