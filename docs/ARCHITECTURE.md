...
Markdown 语法由第三方 markdown-it 提供；src/shared/novel/parser.js 是唯一配置入口，供构建提取 H1 和浏览器渲染共用。esbuild 将解析器及依赖打包为 dist/app/shared/novel/parser.js，并将相关许可证复制到 dist/licenses；访客无需请求 CDN。markdown.js 只把解析结果转换为 DOM，并接入桌面文档导航，不自行解析 Markdown 语法。包内引用以成品根目录为基准，站内文档链接转为桌面章节地址，外部 HTTP(S) 链接使用新标签页。关闭原始 HTML，危险链接由解析器处理。

阅读器目录以清单顺序渲染 H1 标题，不显示 README 文件行；点击书名/卷名只显示 README，加减号单独控制折叠。reader-layout.js 管理目录默认宽度、两栏最小宽度、细分隔线拖动/键盘调整和窗口最小宽度通知。目录没有独立最大值，只受正文最小宽度限制；超窄屏无法容纳两栏时保留内部横向滚动。每个新窗口重新使用默认值，最小化保留当前值。目录按钮按图标与标题收紧，超长标题单行省略，悬停显示虚线。

目录保留原有细分隔线，页签直接定位于正文面板左上角的留白处，不占用独立竖栏。梯形宽度引用 --scrollbar-size，底边采用 45° 斜线，单行双方向字符在短边中点对齐。页签不使用 classic-button；悬停、按住和键盘焦点仅显示虚线轮廓，不使用凹凸、位移或动画。菜单栏保留“文件”“查看”占位菜单和前后翻章箭头；reader-menu.js 管理菜单生命周期。全局翻章方向键只在阅读器前台且未处理其他控件时生效。ResizeObserver 更新末尾留白，使最后一行能滚至顶部；关闭窗口时释放观察器和全局监听。百分比使用视口底部相对 prose 高度的位置，排除末尾留白。shared/status.js 统一用带空格的竖线分隔状态信息；生产者标题内部的标点不修改。正文禁用选中和普通复制事件，仅为基础界面门槛，不影响公开文件可访问性。
...
Pages 工作流仅通过用户手动触发。用户须在远端将 Pages Source 设为 GitHub Actions；工作流先 npm ci 安装锁定依赖，再从已推送的输入运行检查与构建，上传 dist 部署，不将 dist 提交到 Git。代理不执行远端操作，远端验收归用户。
